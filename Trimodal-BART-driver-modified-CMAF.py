...
log_path = make_exp_dirs('bart-driver')
...
GRAPH_INPUT_PATH='../Data/text_adj/'
...
set_random_seed(random.randint(0,9999))#42
#set_random_seed(8840)1681 7948 3838
...
class ContextAwareAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(torch.rand((1, 768), out=None))
    ...
    def forward(self,
        ...
        # k_cap=k_cap* self.weight+k*(1-  self.weight)
        # v_cap = (1 - lambda_v) * v + lambda_v * value_context
        # v_cap=v_cap*  self.weight+v*(1-  self.weight)
        #
        # #modify from BART-GCN
        # k_cap = lambda_k * (1 - self.weight) * k + self.weight * key_context
        # v_cap = lambda_v * (1 - self.weight) * v + self.weight * value_context
        # k_cap = (1 - self.weight) * k + self.weight * key_context
        # v_cap = (1 - self.weight) * v + self.weight * value_context

        #another trick q k for modality fusion
        key_context = self.u_k(context)
        query_context = self.u_v(context)#u_q
        lambda_k = F.sigmoid(self.w1_k(k) + self.w2_k(key_context))
        lambda_q = F.sigmoid(self.w1_v(v) + self.w2_v(query_context))
        k_cap = (1 - lambda_k) * k + lambda_k * key_context
        q_cap=(1 - lambda_q) * q + lambda_q * query_context

        attention_output, _ = self.attention_layer(query=q_cap,
                                                   key=k_cap,
                                                   value=v)
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.acoustic_gc1=GraphConvolution(154,154)
        self.acoustic_gc2=GraphConvolution(154,154)
        self.visual_gc1=GraphConvolution(2048,2048)
        self.visual_gc2=GraphConvolution(2048,2048)
        ...
        self.MAF_layer = MAF(dim_model=embed_dim,
                             dropout_rate=0.2)
        self.attention=Multihead_Attention(768)
        # =============================================================================== #
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                ...
                #calculate the mask
                # visual_mask = torch.ones([bs, visual_input.shape[1]], device=DEVICE, dtype=torch.long)
                # acoustic_mask = torch.ones([bs, acoustic_input.shape[1]], device=DEVICE, dtype=torch.long)

                # ================================ Modifications of GCN ================================ #
                #input the text representations into the GCN
                v_graph=np.diag([96]*1)
                v_graph=torch.tensor(v_graph,device=DEVICE)
                v_graph=v_graph.unsqueeze(0)
                v_graph=torch.cat((v_graph,v_graph),0)
                v_graph=torch.cat((v_graph,v_graph),0)
                text_gcn=hidden_states
                text_adj=graph[:,:,0:480]
                text_gcn_feat=F.relu(self.gc1(text_gcn,text_adj))
                text_gcn_feat = F.relu(self.gc2(text_gcn_feat, text_adj))
                text_gcn_feat=text_gcn_feat+hidden_states
                text_attention=self.attention(text_gcn_feat,text_gcn)
                text_attention=text_attention+text_gcn_feat
                ...
                acoustic_gcn_feat = F.relu(self.acoustic_gc1(acoustic_input, va_graph))#(600,154) (600,600)
                acoustic_gcn_feat = F.relu(self.acoustic_gc2(acoustic_gcn_feat, va_graph))
                # acoustic_gcn_feat=acoustic_gcn_feat.squeeze(0)

                visual_gcn_feat =  F.relu(self.visual_gc1(visual_input, va_graph[:,0:96,0:96]))#(96,2048) (96,96)
                visual_gcn_feat =  F.relu(self.visual_gc2(visual_gcn_feat, va_graph[:, 0:96, 0:96]))
                #this is a 3d tensor,to permute(0,2,1)
                hidden_states = self.MAF_layer(text_input=text_gcn_feat,
                                               acoustic_context=acoustic_gcn_feat,
                                               visual_context=visual_gcn_feat)

#text_input=hidden_states
            # =============================================================================== #
...
class MultimodalBartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        hidden_states=encoder_outputs[0]#(4,480,768)
        # if graph is None:
        #     print("error")
        #     exit()
        # gc_feat=self.gc(hidden_states,graph)
        # fusion_feat=gc_feat+hidden_states
...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        ...
        self.weight = nn.Parameter(torch.eye(in_features))#torch.FloatTensor(in_features, out_features)
        if bias:
    ...
    def forward(self, text, adj):
        ...
        # adj=adj[:,:,0:480]
        denom = torch.sum(adj.float(), dim=2, keepdim=True) + 1
        # print(denom)
...
def prepare_dataset(text_path: str,
    ...
    if unfolded_dialogue:
        ...
        for i in range(1, len(data)+1):
            ...
            adj_i=adj[str(i)]
            adj_all.append(adj_i)
            adj_va_i = np.pad(adj_i, ((0, 120), (0, 120)), 'constant', constant_values=(0, 0))

            adj_va_all.append(adj_va_i)
        ...
        df =  pd.DataFrame(list(zip(dialogue, code_mixed_explanation,adj_all,adj_va_all)),
                            columns=['dialogue', 'code_mixed_explanation','graph','va_graph'])
        ...
        df =  pd.DataFrame(list(zip(context, target, code_mixed_explanation,adj_all,adj_va_all)),
                            columns=['context', 'target', 'code_mixed_explanation','graph','va_graph'])
        del target
        del context
...
def preprocess_dataset(dataset,
    ...
    #input graph
    model_inputs['graph']= torch.tensor([l for l in dataset['graph']], dtype=torch.float, device=DEVICE)
    model_inputs['va_graph']= torch.tensor([l for l in dataset['va_graph']], dtype=torch.long, device=DEVICE)
    del target
    del labels
    gc.collect()
    return model_inputs
...
def get_scores(reference_list: list,
    ...
    for reference, hypothesis in list(zip(reference_list, hypothesis_list)):
        ...
        met += meteor_score([ref_set], hyp_set)
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        input_ids, attention_mask, acoustic_input, visual_input, labels,graph,va_graph = batch
        optimizer.zero_grad()
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        log.info("Epoch: {}\ttrain_loss: {}\tval_loss: {}\tmin_validation_loss: {}".format(epoch+1, train_loss, val_loss, min(val_losses)))
        ...
        score_val.append(val_results)
        score_test.append(test_results)
        if val_results['rouge_1']>max_val['rouge_1']:

         max_val=val_results
        if test_results['rouge_1'] > max_test['rouge_1']:
             max_test = test_results
...
if __name__ == "__main__":
    ...
    MODEL = MultimodalBartForConditionalGeneration.from_pretrained('/data/qiaoyang/ouyangkun/bart/bart-base')
    #MODEL=BartModel.from_pretrained('/data/qiaoyang/ouyangkun/bart/bart-base')
    print("Model loaded...\n")
    MODEL.to(DEVICE)

    TOKENIZER = BartTokenizerFast.from_pretrained('/data/qiaoyang/ouyangkun/bart/bart-base')
    ...
    train_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'train_data_final_plain.json',
                                       ...
                                       graph_path=GRAPH_INPUT_PATH+'train_cos_similarity_ADJ.pkl',
                                       lowercase_utterances = LOWERCASE_UTTERANCES,
                                       unfolded_dialogue = UNFOLDED_DIALOGUE)
    ...
    val_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'val_data_final_plain.json',
                                     ...
                                     graph_path=GRAPH_INPUT_PATH + 'val_cos_similarity_ADJ.pkl',
                                     lowercase_utterances = LOWERCASE_UTTERANCES,
                                     unfolded_dialogue = UNFOLDED_DIALOGUE)
    ...
    test_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'test_data_final_plain.json',
                                      ...
                                      graph_path=GRAPH_INPUT_PATH + 'test_cos_similarity_ADJ.pkl',
                                      lowercase_utterances = LOWERCASE_UTTERANCES,
                                      unfolded_dialogue = UNFOLDED_DIALOGUE)
    print("\nTest Data Loaded...")
    gc.collect()  
