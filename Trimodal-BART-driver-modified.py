    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                ...
                # text_adj=torch.eye(480,device=DEVICE)#np.diag([1]*480)
                # text_adj=torch.unsqueeze(text_adj,0)
                #
                # text_adj=torch.cat((text_adj,text_adj),0)
                text_adj=graph
                text_gcn_feat=F.relu(self.gc1(text_gcn,text_adj))
                text_gcn_feat = F.relu(self.gc2(text_gcn_feat, graph))
...
def prepare_dataset(text_path: str,
    ...
    if unfolded_dialogue:
        ...
        df =  pd.DataFrame(list(zip(dialogue, code_mixed_explanation,adj_all)),
                            columns=['dialogue', 'code_mixed_explanation','graph'])
        ...
        df =  pd.DataFrame(list(zip(context, target, code_mixed_explanation,adj_all)),
                            columns=['context', 'target', 'code_mixed_explanation','graph'])
        del target
        del context
...
def preprocess_dataset(dataset,
    ...
    #input graph
    model_inputs['graph']= torch.tensor([l for l in dataset['graph']], dtype=torch.long, device=DEVICE)
    del target
    del labels
    gc.collect()
    return model_inputs
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        input_ids, attention_mask, acoustic_input, visual_input, labels,graph = batch
        optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    train_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'train_data_final_plain.json',
                                       ...
                                       graph_path=GRAPH_INPUT_PATH+'train_ADJ.pkl',
                                       lowercase_utterances = LOWERCASE_UTTERANCES,
                                       unfolded_dialogue = UNFOLDED_DIALOGUE)
    ...
    val_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'val_data_final_plain.json',
                                     ...
                                     graph_path=GRAPH_INPUT_PATH + 'val_ADJ.pkl',
                                     lowercase_utterances = LOWERCASE_UTTERANCES,
                                     unfolded_dialogue = UNFOLDED_DIALOGUE)
    ...
    test_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'test_data_final_plain.json',
                                      ...
                                      graph_path=GRAPH_INPUT_PATH + 'test_ADJ.pkl',
                                      lowercase_utterances = LOWERCASE_UTTERANCES,
                                      unfolded_dialogue = UNFOLDED_DIALOGUE)
    print("\nTest Data Loaded...")
    gc.collect()  
